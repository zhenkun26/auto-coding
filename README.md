...
一套面向 AI 编码助手的**风险感知交付技能**：按不确定性和操作风险决定执行深度，按需规划、最小改动、用项目自带的工具链验证，最后凭证据交付。

## 它是什么

auto-coding 帮助编码智能体在修改代码时，先识别风险与约束，再以与任务相称的深度完成规划、实现、验证与交付。它由一套在 vibe coding 中反复踩坑后沉淀的并联开发体系（OpenSpec 规划 + Pipeline 执行 + Ponytail 代码最小化 + grill-me 决策追问）重构而来——实战机制全部保留，只是换了一副更克制的骨架：

- **主文件只留契约**：`SKILL.md` 约 180 行，细节全部拆到 12 份按需读取的 references 里；
- **默认零过程文件**：不再产出 TASK_PLAN / LOCATE_MAP / RUN_LOG 等一摞流水线文件，只有长任务才会用到单一状态文件；
- **授权边界前置**：初始化规格系统、安装依赖、提交、部署、迁移、删除，都需要你明确授权。

## 核心理念

不是所有改动都要走同一条重型流水线。本技能按不确定性、影响范围和操作风险决定执行深度：**风险优先于改动规模**——哪怕只改一行，只要涉及鉴权或金额，就属于 High-risk。

它追求**最小而完整**的改动：优先复用现有实现、标准库和已安装的依赖（复用 > stdlib > 已装依赖 > 新代码），避免无关重构、流程文件膨胀和未经授权的副作用。验证证据严格区分 `PASS` / `FAIL` / `BLOCKED` / `NOT_APPLICABLE`，`BLOCKED` 绝不会被当成 `PASS`。
...
Greenfield/brownfield 不影响路由等级，只影响规划和定位的深度（greenfield 没有既有代码可定位，直接创建即可）。
...
4. 按复用梯子实现；每个任务都过 L0（导入）→ L1（行为断言）→ L2（契约比对）三层自检，并带层级类型检查点和逃逸门检测。
5. 用项目已有的命令验证；工具缺失时按 adaptive 规则处理（有配置但没装 → 提示安装并暂停；没配置 → 降级为替代证据并如实标注）。
6. 最后汇报改动、验证证据、逃逸门、假设和后续动作；提交、规格 sync/archive 只作为建议，需经你授权才会执行。

冲突时按既定裁决执行：需求存在性以规格为准，代码复用按复用梯子优先，规格缺陷则回滚并报告缺陷，任务清单以事实为准。详见 [references/conflict-rulings.md](references/conflict-rulings.md)。
...
以上操作都必须先获得你的明确授权。无法执行的验证会标记为 `BLOCKED`，并与替代证据分开说明。

## 持续沉淀与断点恢复

- **默认不沉淀任何过程文件**。唯一的常驻产物是 `ai_pipeline/ERROR_MEMORY.md`，只在发生自愈、逃逸门或 Critical 失败时追加（见 [references/sedimentation.md](references/sedimentation.md)）。
- **断点恢复**：只有长任务或容易被打断的任务才会用到单一状态文件 `ai_pipeline/state.json`，由 `scripts/manage_state.py` 原子化读写（见 [references/recovery.md](references/recovery.md)）。下次调用时如实打印断点，并询问继续还是重来。
...
或者直接描述任务，让技能自己完成路由。已有 OpenSpec 的仓库可以直接交接变更目录：`按 auto-coding 实现 openspec/changes/<name>/`。
...
- 自动化契约检查：用 AST 比对规格签名与实际代码，命名空间感知（`ClassName.method`），空契约不会误报通过。
